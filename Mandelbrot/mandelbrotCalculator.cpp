...
MandelbrotCalculator::MandelbrotCalculator(unsigned int* escapeCounts, int width, int height)
{
    ...
    temporaryResultAVX = (double*)_aligned_malloc(4*sizeof(double), 32); //which boundary?32 or 64?
...
{
    _aligned_free(temporaryResultAVX);
}
...
void MandelbrotCalculator::calculateAVXSerial(unsigned int numberOfIterations, double upperLeftX, double upperLeftY, double downRightX, double downRightY, int canvasWidth, int canvasHeight)
{
    ...
    int wholeParts = canvasWidth / 4; //4 pixels are read at a time (AVX has 256-bit registers, it can fit 4 doubles).It is possible that canvas dimension won't be a multiple of 4
    ...
    for(int y = 0; y < canvasHeight; y++)
    {
        ...
        for(int x = 0; x < wholeParts*4; x += 4)
        {
            ...
            _mm256_store_pd(temporaryResultAVX, divergenceIterations);


            unsigned int first = *((unsigned int*)(temporaryResultAVX));
            unsigned int second = *((unsigned int*)(temporaryResultAVX + 1));
            unsigned int third = *((unsigned int*)(temporaryResultAVX + 2));
            unsigned int fourth = *((unsigned int*)(temporaryResultAVX + 3));
...
void MandelbrotCalculator::calculateAVXParallel(unsigned int numberOfIterations, double upperLeftX, double upperLeftY, double downRightX, double downRightY, int canvasWidth, int canvasHeight)
{
    ...
    //__m256d r0, i0;
    //_mm256_set_pd - load 4 doubles into a register
        //set 4 numbers
